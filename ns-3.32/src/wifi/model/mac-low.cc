...
void MacLow::StartTransmission (Ptr<WifiMacQueueItem> mpdu, MacLowTransmissionParameters params, Ptr<Txop> txop) {
   ...
   */
  if (hdr.IsQosData () && !hdr.GetAddr1 ().IsGroup () && !hdr.IsMoreFragments () && hdr.GetFragmentNumber () == 0) {
      ...
      // Update the current packet at QosTxop, given that A-MSDU aggregation may have been performed on the peeked frame
      ...
      else if (m_currentTxVector.GetMode ().GetModulationClass () == WIFI_MOD_CLASS_VHT || m_currentTxVector.GetMode ().GetModulationClass () == WIFI_MOD_CLASS_HE) {
          // VHT/HE single MPDU
  ...
  NS_LOG_DEBUG ("startTx size=" << m_currentPacket->GetSize () << ", to=" << m_currentPacket->GetAddr1 () << ", txop=" << m_currentTxop);
...
bool MacLow::IsWithinSizeAndTimeLimits (Ptr<const WifiMacQueueItem> mpdu, WifiTxVector txVector, uint32_t ampduSize, Time ppduDurationLimit) {
  NS_ASSERT (mpdu != 0 && mpdu->GetHeader ().IsQosData ());
  return IsWithinSizeAndTimeLimits (mpdu->GetSize (), mpdu->GetHeader ().GetAddr1 (), mpdu->GetHeader ().GetQosTid (), txVector, ampduSize, ppduDurationLimit);
}

bool MacLow::IsWithinSizeAndTimeLimits (uint32_t mpduSize, Mac48Address receiver, uint8_t tid, WifiTxVector txVector, uint32_t ampduSize, Time ppduDurationLimit) {
  ...
  // Cambio David
  if(endOfTransmissionWindow.IsStrictlyPositive()) {
    Time reimainingTxWindow = endOfTransmissionWindow - Simulator::Now();
    if (txTime > reimainingTxWindow) {
      return false;
    }
  }

  if ((ppduDurationLimit.IsStrictlyPositive () && txTime > ppduDurationLimit) || (maxPpduDuration.IsStrictlyPositive () && txTime > maxPpduDuration)) {
...
void MacLow::ForwardDown (Ptr<const WifiPsdu> psdu, WifiTxVector txVector) {
...
MacLow::SendCtsToSelf (void)
{
  ...
  m_sendDataEvent = Simulator::Schedule (txDuration, &MacLow::SendDataAfterCts, this, duration);
}

void MacLow::ReserveMedium (Time duration, bool pauseTransmissions) {
  // Avoids collissions with beacons
  if (m_phy->IsStateTx () || m_phy->IsStateSwitching ()) {
    Simulator::Schedule (NanoSeconds(1), &MacLow::ReserveMedium, this, duration, pauseTransmissions);
    return;
  }
  ...
  cts.SetNoRetry ();
//  cts.SetAddr1 (m_self);
  cts.SetAddr1 (Mac48Address("00:00:00:00:00:00"));

  WifiTxVector ctsTxVector = GetRtsTxVector (*m_currentPacket->begin ());/////////////////////////TODO
  cts.SetDuration (duration);
  ForwardDown (Create<const WifiPsdu> (Create<Packet>(), cts), ctsTxVector);
  NotifyNav (Create<Packet>(), cts);
}
...
bool MacLow::IsCfPeriod (void) const {
  return (m_stationManager->GetPcfSupported () && m_cfpStart.IsStrictlyPositive ());
}

bool MacLow::CanTransmitNextCfFrame (void) const {
  NS_LOG_FUNCTION (this);
  if (!IsCfPeriod ())
    {
      return false;
...
void MacLow::setEndOfTransmissionWindow(Time endOfTransmissionWindow) {
  this->endOfTransmissionWindow = endOfTransmissionWindow;
}
